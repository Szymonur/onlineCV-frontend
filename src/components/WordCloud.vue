...
const drawWordCloud = () => {
  ...
  const mappedKeywords = props.keywords.map((d) => ({
    text: d.keyword.trim(),
    value: d.weight,
  }));
  ...
  const fontSizeScale = d3
    .scaleLinear()
    .domain([1, Math.max(...mappedKeywords.map((d) => d.value))])
    .range([10, 20]);
  ...
  const layout = cloud()
    ...
    .words(mappedKeywords.map((d) => ({ text: d.text, size: fontSizeScale(d.value) })))
    .padding(2)
